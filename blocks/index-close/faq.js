...
function mountFaqItem(questionCell, answerCell) {
  ...
  item.className = 'index-close-faq-item';

  const trigger = document.createElement('button');
  trigger.className = 'index-close-faq-trigger';
  trigger.type = 'button';
  trigger.setAttribute('aria-expanded', 'false');

  const questionText = document.createElement('span');
  questionText.className = 'index-close-faq-question';
  questionText.textContent = questionCell?.textContent.trim() || '';

  const icon = document.createElement('span');
  icon.className = 'index-close-faq-icon';
  ...
  body.className = 'index-close-faq-body';
  body.hidden = true;

  if (answerCell) {
    const inner = document.createElement('div');
    inner.className = 'index-close-faq-body-inner';
...
export function mountFaqSection(parent, data, { standalone = false, skipHead = false } = {}) {
  ...
  section.className = standalone ? 'index-close-faq' : 'index-close-section index-close-section--faq';
  section.id = 'faq';
  ...
  if (!skipHead) {
    section.append(buildSectionHead({
      ...
      title: data.title,
      classPrefix: 'index-close',
    }));
  }

  const list = document.createElement('div');
  list.className = 'index-close-faq-list';
