/**
 * Page Hero — shared detail hero for club and event pages.
 * da.live: key | value config rows; set `type` to `club` or `event`.
 */
import {
  esc,
  getAuth,
...
function resolveType(blockConfig) {
  const explicit = cfg(blockConfig, 'type', '').toLowerCase();
  if (explicit === 'club' || explicit === 'event') return explicit;
  const path = (window.location.pathname || '/').replace(/\/$/, '') || '/';
  if (path === '/club' || path.startsWith('/club/')) return 'club';
...
async function decorateClubHero(block, config) {
  ...
  try {
    ...
    console.error('[page-hero:club]', err);
...
async function decorateEventHero(block, config) {
  ...
  try {
    ...
    console.error('[page-hero:event]', err);
  ...
  ensureEventScripts().then(() => {
    ...
    console.error('[page-hero:event] scripts', err);
  });

  document.dispatchEvent(new CustomEvent('event-hero-ready'));
}

/** Shared event hero mount — used by page-hero and legacy event-hero block. */
export async function decorateEventHeroBlock(block, config = {}) {
  block.innerHTML = '';
  block.classList.add('page-hero', 'page-hero--event', 'event-hero');
  await decorateEventHero(block, { ...EVENT_DEFAULTS, ...config });
}

export default async function decorate(block) {
  const rawConfig = readPageConfig(block, {});
  const type = resolveType(rawConfig);
  const defaults = type === 'event' ? EVENT_DEFAULTS : CLUB_DEFAULTS;
  const config = { ...defaults, ...rawConfig };

  block.innerHTML = '';
  block.classList.add('page-hero');

  if (type === 'event') {
    await decorateEventHeroBlock(block, config);
    return;
  }

  if (type === 'club') {
    block.classList.add('page-hero--club');
    await decorateClubHero(block, config);
    return;
  }

  block.innerHTML = '<p class="ch-not-found">Page Hero: set <code>type</code> to <code>club</code> or <code>event</code> in da.live.</p>';
}
