...
export function mountOrganizerSection(block, ctx) {
  ...
  const sectionTitle = cfg(pageConfig, 'section-organizer', 'Organiser & contact');
  ...
  if (!authed) {
    ...
    body = `
      <section class="event-organizer-section" aria-label="Organiser contact">
        <h2 class="event-about-heading">${esc(sectionTitle)}</h2>
        <div class="event-organizer-locked">
          <p>${esc(cfg(pageConfig, 'organizer-guest-text', 'Organiser contact details are only visible to signed-in Adobe Clubs members.'))}</p>
          <a class="event-organizer-signin" href="${esc(loginUrl)}">${esc(cfg(pageConfig, 'organizer-sign-in', 'Sign in to view contact details'))}</a>
        </div>
      </section>`;
  } else {
    ...
    const rows = [
      `<div class="event-organizer-row">
        <span class="event-organizer-label">${esc(cfg(pageConfig, 'hosting-club-label', 'Hosting club'))}</span>
        <span class="event-organizer-value">${esc(clubName)}</span>
      </div>`,
    ];
    if (slackLabel) {
      rows.push(`<div class="event-organizer-row">
        <span class="event-organizer-label">${esc(cfg(pageConfig, 'slack-channel-label', 'Slack channel'))}</span>
        ${slackUrl
    ...
    rows.push(`<div class="event-organizer-row">
      <span class="event-organizer-label">${esc(cfg(pageConfig, 'clubs-admin-label', 'Clubs admin'))}</span>
    ...
    body = `
      <section class="event-organizer-section" aria-label="Organiser contact">
        <h2 class="event-about-heading">${esc(sectionTitle)}</h2>
        <p class="event-organizer-intro">${esc(cfg(pageConfig, 'organizer-intro', "Questions about this event? Reach out to the organisers below — we're happy to help."))}</p>
