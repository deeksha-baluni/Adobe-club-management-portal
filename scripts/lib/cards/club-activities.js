...
function renderTitle(title) {
  return `<h2 class="club-section-title">${esc(title)}</h2>`;
}

export function mountClubActivities(block, ctx) {
  const { club, pageConfig = {} } = ctx;
  const meta = getMeta(club.id);
  const title = cfg(pageConfig, 'section-activities', cfg(pageConfig, 'title', 'What this club does'));
  const showTitle = !hasSiblingSectionHead(block);
  block.innerHTML = `
    <div class="club-cards-body club-cards-body--activities">
      ${showTitle ? renderTitle(title) : ''}
