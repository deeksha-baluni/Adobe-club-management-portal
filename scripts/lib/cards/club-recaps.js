...
export function closeRecapReadModal() {
  const modal = document.getElementById('club-recap-modal');
...
export async function ensureRecapDeps() {
  const base = window.hlx?.codeBasePath || '';
  await loadScript(`${base}/scripts/user-features.js`);
}

export function mountClubRecaps(block, ctx) {
  ...
  const title = cfg(pageConfig, 'section-recaps', cfg(pageConfig, 'title', 'Highlights from recent sessions'));
  const recapCta = cfg(pageConfig, 'recap-cta', 'Post a recap');
  const showTitle = !hasSiblingSectionHead(block);

  block.innerHTML = `
    <div class="club-cards-body club-cards-body--recaps" id="club-recaps">
      ${showTitle ? `<h2 class="club-section-title">${esc(title)}</h2>` : ''}
